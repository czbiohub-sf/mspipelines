...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component sync_resources_test")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "sync_resources_test." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process sync_resources_test_process {
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.sync_resources_test.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow sync_resources_test {
  ...
  result_ = sync_resources_test_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.sync_resources_test
    .arguments
  ...
  result = sync_resources_test(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.sync_resources_test.output = "sync_resources_test.log"

  Channel.from(rootDir) \
    | filter { params.sync_resources_test.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.sync_resources_test.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | sync_resources_test

  emit:
  sync_resources_test.out
}