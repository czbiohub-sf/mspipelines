  ...
  "arguments" : [
    {
      ...
      "default" : [
        "s3://czbiohub-pipelines/resources_test_ms"
      ],
      "required" : false,
      "direction" : "input",
      "multiple" : false,
  ...
  "resources" : [
    {
      ...
      "path" : "script.sh",
      "is_executable" : true,
      "parent" : "file:/home/runner/work/mspipelines/mspipelines/src/download/sync_test_resources/config.vsh.yaml"
    }
  ],
  "description" : "Synchronise the test resources from s3://czbiohub-pipelines/resources_test_ms to resources_test",
  "usage" : "sync_test_resources\nsync_test_resources --input s3://czbiohub-pipelines/resources_test_ms --output resources_test\n",
  "test_resources" : [
    {
      "type" : "bash_script",
      "path" : "run_test.sh",
...
$( if [ ! -z ${VIASH_PAR_QUIET+x} ]; then echo "${VIASH_PAR_QUIET}" | sed "s#'#'\\"'\\"'#g" | sed "s#.*#par_quiet='&'#" ; fi )
$( if [ ! -z ${VIASH_PAR_DRYRUN+x} ]; then echo "${VIASH_PAR_DRYRUN}" | sed "s#'#'\\"'\\"'#g" | sed "s#.*#par_dryrun='&'#" ; fi )
$( if [ ! -z ${VIASH_PAR_DELETE+x} ]; then echo "${VIASH_PAR_DELETE}" | sed "s#'#'\\"'\\"'#g" | sed "s#.*#par_delete='&'#" ; fi )
$( if [ ! -z ${VIASH_PAR_EXCLUDE+x} ]; then echo "${VIASH_PAR_EXCLUDE}" | sed "s#'#'\\"'\\"'#g" | sed "s#.*#par_exclude='&'#" ; fi )
...
thisDefaultProcessArgs = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "czbiohub/mspipelines/download_sync_test_resources",
    "tag" : "main_build"
  }
}'''),
  // auto settings
