...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component rawbeans")
      println("exiting now...")
        exit 1
    }
  }
...
process rawbeans_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.rawbeans.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow rawbeans {
  ...
  result_ = rawbeans_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.rawbeans
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.rawbeans.output = "rawbeans.log"

  Channel.from(rootDir) \
    | filter { params.rawbeans.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.rawbeans.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | rawbeans
