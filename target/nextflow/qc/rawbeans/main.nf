...
thisHelpMessage = '''rawbeans main_build

Perform a QC analysis.

Usage:
rawbeans --input file1.mzML --output out_qc/
...
Options:
    ...
    --output
        ...
        example: output_qc_dir
        An output directory to the QC results.

    --batch
        type: boolean_true
        When specified, will generate one report per sample instead of one
report for all samples.

    --no_report
        type: boolean_true
        When specified, will only generate json and no report.

    --mass
        type: double, multiple values allowed
        List of masses to follow. Example: '--mass 371.10124 --mass 445.12003'
or '--mass 371.10124:445.12003'.'''
...
$( if [ ! -z ${VIASH_PAR_BATCH+x} ]; then echo "par_batch='${VIASH_PAR_BATCH//\\'/\\'\\"\\'\\"\\'}'"; fi )
$( if [ ! -z ${VIASH_PAR_NO_REPORT+x} ]; then echo "par_no_report='${VIASH_PAR_NO_REPORT//\\'/\\'\\"\\'\\"\\'}'"; fi )
$( if [ ! -z ${VIASH_PAR_MASS+x} ]; then echo "par_mass='${VIASH_PAR_MASS//\\'/\\'\\"\\'\\"\\'}'"; fi )
...
thisDefaultProcessArgs = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "czbiohub/mspipelines/qc_rawbeans",
    "tag" : "main_build"
  }
}"""),
  // auto settings
