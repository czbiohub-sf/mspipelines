...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component maxquant")
      println("exiting now...")
        exit 1
    }
  }
...
process maxquant_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.maxquant.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow maxquant {
  ...
  result_ = maxquant_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.maxquant
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.maxquant.output = "maxquant.log"

  Channel.from(rootDir) \
    | filter { params.maxquant.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.maxquant.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | maxquant
