...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component maxquant_to_h5ad")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "maxquant_to_h5ad." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process maxquant_to_h5ad_process {
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.maxquant_to_h5ad.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow maxquant_to_h5ad {
  ...
  result_ = maxquant_to_h5ad_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.maxquant_to_h5ad
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.maxquant_to_h5ad.output = "maxquant_to_h5ad.log"

  Channel.from(rootDir) \
    | filter { params.maxquant_to_h5ad.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.maxquant_to_h5ad.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | maxquant_to_h5ad

  emit:
  maxquant_to_h5ad.out
}