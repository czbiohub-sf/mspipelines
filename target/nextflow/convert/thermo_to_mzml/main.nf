...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component thermo_to_mzml")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "thermo_to_mzml." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process thermo_to_mzml_process {
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.thermo_to_mzml.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow thermo_to_mzml {
  ...
  result_ = thermo_to_mzml_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.thermo_to_mzml
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.thermo_to_mzml.output = "thermo_to_mzml.log"

  Channel.from(rootDir) \
    | filter { params.thermo_to_mzml.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.thermo_to_mzml.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | thermo_to_mzml
