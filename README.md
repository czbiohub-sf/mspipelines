...
Build container
```sh
bin/viash run src/maxquant/maxquant/config.vsh.yaml -- ---setup cb
```
...
bin/viash run src/maxquant/maxquant/config.vsh.yaml -- \
  --input "$resources_test/Raw/" \
  --reference "$resources_test/Fasta/20211015_Kistler_Human.Cow.ZEBOV_NP_P2A_VP35_P2A_VP30.fasta" \
  --output output
```
